...
describe Mixpanel::Tracker do
  ...
  context "Parsing special Person properties" do
    it "should parse last_login to $last_login" do
      @mixpanel.parse_special_person_properties({:last_login => "1"}).keys.first.should == "$last_login"
    end
  end
end
